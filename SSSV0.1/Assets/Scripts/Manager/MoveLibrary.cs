...
public class MoveLibrary : MonoBehaviour
{
    ...
    private List<Move> punchMoves = new List<Move>();
    private List<Move> kickMoves = new List<Move>();
    private List<Move> specialMoves = new List<Move>();
    private List<Move> movementMoves = new List<Move>();
    ...
    void InitializeMoves()
    {
        ...
        // HIGH GUARD
        specialMoves.Add(new Move("High Guard", MoveCategory.HighGuard,
            Speed: 10, Power: 0, Energy: 0, Recovery: 0,
            optDist: 0, minDist: 0, maxDist: 0));

        // LOW GUARD
        specialMoves.Add(new Move("Low Guard", MoveCategory.LowGuard,
            Speed: 10, Power: 0, Energy: 0, Recovery: 0,
            optDist: 0, minDist: 0, maxDist: 0));

        // WAIT
        specialMoves.Add(new Move("Wait", MoveCategory.Wait,
            Speed: 10, Power: 0, Energy: 0, Recovery: 0,
            optDist: 0, minDist: 0, maxDist: 0));

        // STANCE SWITCH
        specialMoves.Add(new Move("Switch Stance", MoveCategory.StanceSwitch,
    ...
    public List<Move> GetAvailableMoves(Fighter fighter, int currentDistance)
    {
        ...
        foreach (Move move in punchMoves)
        {
            if (move.energy <= fighter.stamina)
                available.Add(move);
        }

        foreach (Move move in kickMoves)
        {
            if (move.energy <= fighter.stamina)
                available.Add(move);
        }
        ...
        {
            if (move.energy > fighter.stamina) continue;

            bool isClosing = (move.moveName.Contains("In") || move.moveName.Contains("Forward"));

            if (isClosing && currentDistance <= 0) continue;
            if (!isClosing && currentDistance >= 8) continue;
